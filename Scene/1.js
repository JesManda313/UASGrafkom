...
function moveCharacterTo(player, target, delta, camera, checkCollision){
    ...
    if(totalDist > 0.01){
        ...
        const needsAvoidance = (checkCollision && (player.colorHex === COLORS[5] || player.colorHex === COLORS[8]));
        ...
        if(needsAvoidance){
            ...
            if(directResult.moved < moveDist * 0.5){
                // Left 45 deg
                const leftDir = dir.clone().applyAxisAngle(new THREE.Vector3(0,1,0), Math.PI/4);
                const leftResult = tryMove(oldPos, leftDir, moveDist, checkCollision);

                // Right 45 deg
                const rightDir = dir.clone().applyAxisAngle(new THREE.Vector3(0,1,0), -Math.PI/4);
                const rightResult = tryMove(oldPos, rightDir, moveDist, checkCollision);

                if(leftResult.moved > directResult.moved && leftResult.moved > rightResult.moved){
...
export async function initializeScene1(scene, createPlayerFunc, playerObj, camera, checkCollision){
    if(!playerObj) return;
    isActive = true;
    sceneTimer = 0;
...
export function updateScene1(delta){
    ...
    if(sceneTimer >= 2.7){
        ...
        if(yellow && !yellow.targetPos && yellow.pathQueue.length === 0 && !yellow.hasPathAssigned){
    ...
    scene1PlayerObjects.forEach(player => {
        ...
        if(player.targetPos){
            const reached = moveCharacterTo(player, player.targetPos, delta, sceneCamera, sceneCheckCollision); 
            if(reached){
                player.targetPos = null; 
            }
        }
    ...
    if(document.getElementById("pos")) document.getElementById("pos").innerText = "Timer: " + sceneTimer.toFixed(2);
    if(document.getElementById("dir")) document.getElementById("dir").innerText = "Scene 1 Playing";
}
