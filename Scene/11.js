...
export async function initializeScene11(
  scene,
  createPlayerFunc,
  playerObj,
  camera
...
export function updateScene11(delta) {
  ...
  if (scene11Player && scene11Player.mesh) {
    ...
    // Phase 2: 1s - 2s (Look Left/Right + Camera Move)
      ...
      if (sceneCamera) {
        sceneCamera.position.x =
          CAM_START_X + pulse * (CAM_PEAK_X - CAM_START_X);
        sceneCamera.position.z = CAM_START_Z;
      }
    ...
    // Phase 3: 2s - 5s (Move to Target)
      ...
      const dir = new THREE.Vector3()
        .subVectors(PLAYER_TARGET_POS, PLAYER_START_POS)
        .normalize();
      ...
      if (sceneCamera) {
        if (sceneTimer >= CAM_MOVE_START_TIME) {
          ...
          sceneCamera.position.x =
            CAM_START_X + (CAM_END_X - CAM_START_X) * tCam;
          sceneCamera.position.z =
            CAM_START_Z + (CAM_END_Z - CAM_START_Z) * tCam;
        } else {
